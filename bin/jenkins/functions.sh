...
deploy () {
  ...
  docker exec -i ${PROJECT}_dev_1 su vagrant -c "
    source /vagrant/bin/init_vagrant_env &&
    make clean_appengine > /dev/null &&
    make appengine > /dev/null &&
    deploy_appengine deploy_settings_grc_dev.sh > /dev/null &&
    appcfg.py --version=${ghprbActualCommit} --oauth2 --oauth2_credential_file=auth update src/"
  print_line
}
